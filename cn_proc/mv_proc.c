...
void handle_msg (struct cn_msg *cn_hdr)
{
  ...
  char pcid[10];
  char *search_cmd = "/usr/java/default/bin/java -cp /opt/spark/conf/:/opt/spark/jars/*:/etc/hadoop/conf/ -Xmx1g org.apache.spark.deploy.SparkSubmit --master spark://172.16.0.108:7077 --class com.ibm.sparktc.sparkbench.cli.CLIKickoff /opt/spark-bench_2.1.1_0.2.2-RELEASE/lib/spark-bench-2.1.1_0.2.2-RELEASE.jar /tmp/spark-bench-7820949217911436513.conf ()";///tmp/spark-bench-5008863244920987380.conf ()
  char proc_cmdline[1000] = "";
  ...
  */
  case PROC_EVENT_EXEC:
    ...
    strcat(proc_cmdline, cmdline(ev->event_data.fork.parent_pid) );
    ...
    while(proc_cmdline[i]){
      printf("%d ",proc_cmdline[i]);
      i++;
    }
    i = 0;
    printf("\n------------\n");
    while(search_cmd[i]){
      printf("%d ",search_cmd[i]);
      i++;
    }
    printf("\n-----\n");
